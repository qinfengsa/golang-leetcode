...
func Constructor() Twitter {
	return Twitter{globalId: 0, userMap: make(map[int]*User)}
}

func (this *Twitter) getUser(userId int) *User {
	var user *User
	if v, ok := this.userMap[userId]; ok {
		user = v
	} else {
		user = &User{
			userId:    userId,
...
func (this *Twitter) PostTweet(userId int, tweetId int) {
	...
	user.tweetList = append(user.tweetList, tweet)
}

// GetNewsFeed
// 检索当前用户新闻推送中最近 10 条推文的 ID 。新闻推送中的每一项都必须是由用户关注的人或者是用户自己发布的推文。
// 推文必须 按照时间顺序由最近到最远排序 。
func (this *Twitter) GetNewsFeed(userId int) []int {
	...
	for k, v := range user.followee {
		if v {
			if followee, ok := this.userMap[k]; ok {
				tweetList = append(tweetList, followee.tweetList...)
			}
		}
	}
	sort.Slice(tweetList, func(i, j int) bool {
		return tweetList[i].time > tweetList[j].time
	})
	result := make([]int, 0)
	for i := 0; i < min(10, len(tweetList)); i++ {
		result = append(result, tweetList[i].tweetId)
	}
	return result
}
...
func (this *Twitter) Unfollow(followerId int, followeeId int) {
	follower := this.getUser(followerId)
	follower.followee[followeeId] = false
}
