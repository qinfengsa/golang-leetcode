...
func validateStackSequences(pushed []int, popped []int) bool {
	...
	for i := 0; i < n; i++ {
		...
		for stack.Len() > 0 {
			back := stack.Back()
			if back.Value.(int) == popped[index] {
				stack.Remove(back)
				index++
			} else {
				break
...
func sumSubarrayMins(arr []int) int {
	...
	for _, num := range arr {
		...
		back := stack.Back()
		for stack.Len() > 0 {
			node := back.Value.(RepInt)
			// 当前num是最大的
			if node.val < num {
				break
			}
			count += node.count
			tmp -= node.val * node.count
			stack.Remove(back)
			back = stack.Back()
		}
		stack.PushBack(RepInt{num, count})
		// 子数组
		tmp += num * count
		result += tmp
		result %= MOD
...
func backspaceCompare(s string, t string) bool {
	...
	var buildStr = func(str string) string {
		result := []byte{}
		for i := range str {
			if str[i] != '#' {
